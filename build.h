...
void create_children(const Node& parent, vector<Node>& tree,uint* m_index);
...
const float* mass,const int N,const int k, float* xsorted,float*ysorted,float* mass_sorted)
{
    ...
    for(Node& nd: tree){
        if(nd.part_end-nd.part_start > k) create_children(nd,tree,m_label);
    }
...
void create_children(const Node& parent, vector<Node>& tree,uint* m_index)
{
   ...
   for(int i=0;i<4;i++) {
       ...
       while((m_index[current_idx] & mask) == child->morton_id) current_idx++;
